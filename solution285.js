...
//? Parameters: a string of random upper and lower case letters

//? Return: a string that is case sensitive and repeats x amount if times depending on the index and is join('-') with
...
function accum(s) {  //! returning to this kata and added to anki
	const strSplitToUpper = s.split('').map((item, index) => item.toUpperCase() + item.toLowerCase().repeat(index))

    return strSplitToUpper.join('-')
}
