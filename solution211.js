...
//? Return: original str and the first two letters in the str
...
//? split('')
//? slice(0,2).join('').split(',')
//? declare var name to arr
//? concat(twoLetters)
const assert = require('assert')

function whoIsPaying(name){
    const twoLetters = name.split('').slice(0,2).join('').split(',')
    const arrName = Array(name)
    return arrName.concat(twoLetters)
  }
console.log(whoIsPaying(""))

assert.strictEqual(whoIsPaying("Mexico"),["Mexico", "Me"]);  //! changed tests didn't work
assert.strictEqual(whoIsPaying("Melania"),["Melania", "Me"]);
assert.strictEqual(whoIsPaying("Melissa"),["Melissa", "Me"]);
