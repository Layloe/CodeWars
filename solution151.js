...
//*Psuedocode: loop and find str includes all cases


function isLockNessMonster(s) {
    const testMonies = s.split(' ').filter(item => (item.includes('tree') || item.includes('fiddy') || item.includes('3.50') ? true : false) )
    const testJoin = testMonies.join(' ')
    return testJoin.includes('tree fiddy') || testJoin.includes('3.50')
  }
