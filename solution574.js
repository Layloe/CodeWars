...
function fix(paragraph){
    const test = paragraph.split('.').map(item => item.toUpperCase())
    return test
}
console.log(fix("hello. my name is inigo montoya. you killed my father. prepare to die."))
// Input is a string/paragraph.
// Output is the same string with the start of a sentence capitalized. 
// Example is above.
// Split with '.' then use index location to uppercase. Use if else on edge case. If 