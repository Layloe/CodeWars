...
function modifyMultiply (str,loc,num) {
    const findWord = str.split(' ').find((item, index) => item.indexOf(loc))
    return findWord
    } 
    console.log(modifyMultiply("This is a string", 3 ,5))
    // input will be three parameters one is as string of words an the other two numbers
    // output will be a string word that is the index and is repeated by the last parameter.
...
//   Test.assertEquals(modifyMultiply("Everything happening around me is very random. I am enjoying the phase, as the journey is far more enjoyable than the destination.",2 ,5), "around-around-around-around-around");
    //split the string then map to access index and then use repeat